...
def get_word_info(word: str) -> List[Dict]:
    """Get detailed information about the word using OpenAI, handling multiple meanings."""
...
def create_cloze_text(catalan_example: str, spanish_example: str, word: str) -> str:
    ...
    # Find the word in the Catalan example
    catalan_cloze = catalan_example.replace(word, f"{{{{c1::{word}}}}}")
    # Find the word in the Spanish example
    spanish_cloze = spanish_example.replace(word, f"{{{{c2::{word}}}}}")
...
def create_csv_files(words: List[str], words_per_file: int = 1500, max_words: int = None):
    ...
    for file_num in range(total_files):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            total_cards = 0
            for word_idx, word in enumerate(file_words, start=start_idx + 1):
                logging.info(f"Processing word {word_idx}/{len(words)}: {word}")
                word_meanings = get_word_info(word)
